...
export class GldriveService {
  ...
  async uploadFiles(files: Express.Multer.File[]) {
    ...
    for (const file of files) {
      const fileMetadata = {
        name: file.originalname,
        parents: ['1qN4pAUf-jkR0xzhbDTDbAzDtxxb8INjx'], // Optional: specify folder ID
      };
  ...
  async uploadPdfToDrive(pdfBuffer: Buffer, filename: string) {
    const fileMetadata = {
      name: filename,
      parents: ['1qN4pAUf-jkR0xzhbDTDbAzDtxxb8INjx'],
    };
